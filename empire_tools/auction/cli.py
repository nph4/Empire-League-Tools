...
def build_initial_state(config: dict, teams: list, free_agents: list) -> DraftState:
    budget = config["auction"]["budget_per_manager"]
    roster_spots = config["auction"]["roster_spots_per_manager"]

    managers = {
        team.team_name: Manager(name=team.team_name, budget=budget) for team in teams
    }
    available = {
...
class AuctionShell(cmd.Cmd):
    ...
    def do_targets(self, arg):
        ...
        if manager_name not in self.state.managers:
            ...
            return
        for player_name, bid in suggest_targets(self.state, self.value_pool, manager_name):
            print(f"{player_name:<25} suggested_bid={bid}")
...
def main():
    ...
    state = build_initial_state(config, league.teams, free_agents)
